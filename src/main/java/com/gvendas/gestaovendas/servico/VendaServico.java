...
public class VendaServico extends AbstractVendaServico {
	...
	public ClienteVendaResponseDTO listarVendaPorCodigo(Long codigoVenda) {
		...
		// metodo "retornandoClienteVendaResponseDto" sendo acessado atraves da classe
		// abstrata
	}

	// Essa anotacao "Transactional" configuramos um rollback, caso ocorra uma excecao, ou seja, a transacao sera efetivada
	// somente se nao ocorrer nenhum problema. Usado OBRIGATORIO quando se trabalha com mais de uma tabela.
	...
	public ClienteVendaResponseDTO atualizar(Long codigoVenda, Long codigoCliente, VendaRequestDTO vendaDto) {
		validarClienteVendaExiste(codigoCliente);
		Cliente cliente = validarClienteVendaExiste(codigoCliente);
	...
	private void validarQuantidadeProdutoExiste(Produto produto, Integer qtdVendaDto) {
		if (!(produto.getQuantidade() >= qtdVendaDto)) {
			throw new RegraNegocioException(
					String.format("A quantidade %s informada para o produto %s nao esta disponivel em estoque.",
	...
	private Venda validarVendaExiste(Long codigoVenda) {
		...
		if (venda.isEmpty()) {
			throw new RegraNegocioException(String.format("Venda de codigo %s nao encontrada.", codigoVenda));
		}
		return venda.get();
	}
	...
	private Cliente validarClienteVendaExiste(Long codigoCliente) {
		...
		if (cliente.isEmpty()) {
			throw new RegraNegocioException(
					String.format("O Cliente de codigo %s informado nao existe no cadastro.", codigoCliente));
		}
		return cliente.get();
	}
